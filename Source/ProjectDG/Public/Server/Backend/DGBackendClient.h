...
DECLARE_DELEGATE_TwoParams(FDGCreateCharacterApiResultCallback, bool /*bSuccess*/, const FDGCreateCharacterResult& /*Result*/);
...
private:
	...
	void SendCreateCharacterPostRequest(
		const FString& EndPoint,
		const FString& BodyJson,
		FDGCreateCharacterApiResultCallback Callback
	);
	...
	static FString BuildCreateCharacterJson(const FDGCreateCharacterRequest& RequestData);
	...
	static FDGCreateCharacterResult ParseCreateCharacterResult(
		bool bRequestSucceeded,
		int32 ResponseCode,
		const FString& ResponseBody
	);

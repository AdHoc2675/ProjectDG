...
void UDG_EnemyAttributeSet::PreAttributeChange(const FGameplayAttribute& Attribute, float& NewValue)
{
	...
	// 그로기 게이지는 0과 MaxStaggerGauge 사이로 고정(Clamp)
	if (Attribute == GetStaggerGaugeAttribute())
	{
		NewValue = FMath::Clamp(NewValue, 0.0f, GetMaxStaggerGauge());
	}
}
...
void UDG_EnemyAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetStaggerGaugeAttribute())
	{
		float Clamped = FMath::Clamp(GetStaggerGauge(), 0.f, GetMaxStaggerGauge());
		SetStaggerGauge(Clamped);

		if (Clamped >= GetMaxStaggerGauge())
		{
...
void UDG_EnemyAttributeSet::OnRep_StaggerGauge(const FGameplayAttributeData& OldStaggerGauge)
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UDG_EnemyAttributeSet, StaggerGauge, OldStaggerGauge);
}
